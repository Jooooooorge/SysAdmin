...
4) Crear imagen Apache
5) Ejecutar imagen personalizada
6) Configurar PostgreSQL (pg1 y pg2)
...
12) Salir
EOF
  read -p "Opción [1-12]: " opt
  case $opt in
    1) install_docker ;; 2) start_apache ;; 3) modify_index ;; 4) build_image ;; 5) run_custom ;; 6) setup_postgres ;; 7) seed_data ;; 8) authlogin_pg1 ;; 9) authlogin_pg2 ;; 10) test_connectivity ;; 11) full_setup ;; 12) echo "Adiós!" && exit 0 ;; *) echo "Inválida" ;;
  esac
done
